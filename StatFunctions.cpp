...
#define one 1.0e0
  ...
  // The following code was added to avoid problems with pointers.
	int gg;
	status = &gg;
...
// The following 4 lines were added by Leigh Johnson to avoid problems with pointers.
	int gg;
	status = &gg;
	double hh;
	bound = &hh;
...
//
		...
		// Code in next line modified from original ("&& ierr != -8" added).
        if(ierr < 0.0e0 && ierr != -8) {
            *status = 10;
            return;
        }
        else  {
...
//
{
  // The following 2 lines were added by Leigh Johnson to avoid problems with pointers.
	double gg;
	ccum = &gg;
...
//The following code was added to avoid pointer problems.
	double gg;
	qans = &gg;
